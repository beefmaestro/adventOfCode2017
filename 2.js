...
//Part 1
// rows.forEach((row) => {
//   let valuesArr = row.split(' ');
//
//   let biggest = Math.max(...valuesArr);
//   let smallest = Math.min(...valuesArr);
//
//   let checksum = biggest - smallest;
//   sum += checksum;
// });
...
rows.forEach((row) => {
  let valuesArr = row.split(' ');
  var found = false;
  console.log(`~~~~~~~~~~~~~~~~~~~~~~~New Row ${rowNum}~~~~~~~~~~~~~~~~~~~~~~~~ `.red)
  rowNum++;

  while (found === false && valuesArr.length >= 1) {
    var testNum = valuesArr.splice(0,1)[0];
    ...
    valuesArr.forEach((val) => {
      ...
      if (Number.isInteger(divided)) {
        ...
        console.log(colors.green('val', val));
        console.log(colors.green('testNum', testNum));
        console.log(colors.green('divided', divided));
      } else {
        log(`testNum ${testNum} \n val ${val} \n  divided ${divided}`);
      }
    });
